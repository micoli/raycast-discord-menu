...
The menu bar icon shows the screen share, microphone, sound and voice member count states (refreshed within ~3s when the discord state changes, through a raycast deeplink sent by the injected script, and every minute as a fallback). Icons (2x2 grid: logo or screen share, microphone, sound, member count) are generated by `npm run build-icons` (needs `rsvg-convert`) and committed in `assets/menu-icons`.
