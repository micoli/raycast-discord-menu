...
The menu bar icon shows the screen share, microphone, sound and voice member count states (refreshed by the menu bar background refresh, every 10s). Icons (2x2 grid: logo or screen share, microphone, sound, member count) are generated by `npm run build-icons` (needs `rsvg-convert`) and committed in `assets/menu-icons`.
